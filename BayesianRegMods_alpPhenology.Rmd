...
```{r}
mod1 <- brm(ordinal_date ~ seasonYear + tmean.avg + prevWintPPT + occID + (1 | scientific_name), data=phen.abio2, family=poisson())
summary(mod1)

#need more iterations
mod1b <- brm(ordinal_date ~ seasonYear + tmean.avg + prevWintPPT + occID + (1 | scientific_name), 
             data=phen.abio2, 
...
mod1c <- brm(ordinal_date ~ seasonYear + tmean.avg * prevWintPPT + occID + (1 | scientific_name), 
             data=phen.abio2, 
             family=gaussian(),#changed to gaussian from poisson to see if it helps with convergence [it helps a TON]
             iter = 4000, 
             chains = 4)
