...
phen.abio2 <- phen.abio %>%
  select(occID, scientific_name, ordinal_date, site, elevation_m, data_type, tmean.avg, ppt.mean, seasonYear, prevWintPPT, prevWintTemp, prevSpringTemp, springFrost)
str(phen.abio2)
names(phen.abio2)
...
PCA_biplot <- ggbiplot(pca_result, obs.scale = 1, var.scale = 1, 
         groups = factor(phen.abio2$data.type),  # Replace with your grouping variable if applicable
...
#Models
From the PCA above, it seems like PC1 is most strongly impacted by temperature (tmean.average) and preceding winter and (or?) spring temperature and year (seasonYear). PC2 suggests that we might also want to include previous winter precipitation (prevWintPPT) and general precipitation (ppt.mean). 
...
#MOD1 versions (i.e. prevWintPPT and tmean.avg)
